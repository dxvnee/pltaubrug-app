package org.d3if3121.AdminAbsen.data.repository
...
class MahasiswaListRepository (
    ...
    override suspend fun addMahasiswa(mahasiswa: Mahasiswa) = try {
        val mahasiswaSama = mahasiswaRef.whereEqualTo("nim", mahasiswa.nip).get().await()
    ...
    override suspend fun updateMahasiswa(mahasiswa: MahasiswaEdit) = try {
        val querySnapshot = mahasiswaRef.whereEqualTo("nim", mahasiswa.nip).get().await()
