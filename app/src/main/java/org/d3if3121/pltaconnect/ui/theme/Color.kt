package org.d3if3121.pltaconnect.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

object Warna {
    val MerahTua = Color(0xFF0797DA)
    val BiruNormal = Color(0xFFFF0000)
    val MerahNormal = Color(0xFF2A2E78)
    val PutihNormal = Color(0xFFFFFFFF)
    val HitamNormal = Color(0xFF000000)
    val AbuNormal = Color(0xFFC3C3C3)
    val AbuTua = Color(0xFFB5B5B5)
    val AbuMuda = Color(0xFFE6E6E6)
    val HijauNormal = Color(0xFF16C900)
    val HijauMuda = Color(0xFF5ED200)
    val KuningMuda = Color(0xFFF5D400)
}
