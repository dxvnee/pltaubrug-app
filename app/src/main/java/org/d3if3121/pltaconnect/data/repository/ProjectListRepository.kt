...
class ProjectListRepository (
    ...
    override fun getRequestList(nim: String) = callbackFlow {
        val listener = pegawaiRef.whereEqualTo("nim", nim)
            .addSnapshotListener { snapshot, e ->
                if (e != null) {
    ...
    override suspend fun addRequest(projectId: String, nim: String) = try {
        ...
        val process2 = pegawaiRef.whereEqualTo("nim", nim).get().await().documents.first()
    ...
    override suspend fun addAccept(projectId: String, nim: String) = try {
        ...
        val process4 = pegawaiRef.whereEqualTo("nim", nim).get().await().documents.first()
    ...
    override fun getProjectListUser(nim: String) = callbackFlow {
        val pegawaiQuery = pegawaiRef.whereEqualTo("nim", nim)
        var projectListener: ListenerRegistration? = null
        var viewedProjects: List<String> = emptyList()

        val pegawaiListener = pegawaiQuery.addSnapshotListener { pegawaiSnapshot, pegawaiError ->
            if (pegawaiSnapshot != null && !pegawaiSnapshot.isEmpty) {
                viewedProjects = pegawaiSnapshot.documents.firstOrNull()?.get("viewedProjects") as? List<String> ?: emptyList()
                ...
                trySend(Response.Failure(pegawaiError))
            }
        }


        awaitClose {
            pegawaiListener.remove()
    ...
    override suspend fun deleteProject(id: String) = try {
        ...
        val querySnapshot = pegawaiRef.whereArrayContains("requests", id).get().await()
